...
#pragma warning restore VSTHRD100 // Avoid async void methods
    ...
    private async Task CreateVisualsAsync(ITextViewLine line, int lineNumber)
    {
        ...
        static string GetDisplayTextFromDoc(XmlDocument xDoc, string key)
        {
            string result = null;
            ...
            if (indexes.Any())
            {
                ...
                foreach (var matchIndex in indexes)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(displayText) && TextSize > 0)
                    {
                        ...
                        var height = (TextSize * Constants.TextBlockSizeToFontScaleFactor) + CSharpReadAssistPackage.Instance?.Options.TopPadding ?? 0 + CSharpReadAssistPackage.Instance?.Options.BottomPadding ?? 0;
    ...
    private IEnumerable<XmlDocument> GetLocalizerDocsOfInterest(string filePathName, List<(string path, XmlDocument xDoc)> xmlDocs, string preferredCulture)
    {
        ...
        static (string, string) StripMatchingStart(string resPath, string codePath)
        {
