...
#pragma warning restore VSTHRD100 // Avoid async void methods
	...
	public static async Task<List<(int Index, string DisplayText)>> GetSubstringsToAdornAsync(string source)
	{
		...
		try
		{
			...
			{
				if (value == AndTerm && (char.IsWhiteSpace(source[index - 1]) || source[index - 1] == '('))
				{
				...
				if (value == NotTerm)
				{
					if (source[index - 1] == '(' || char.IsWhiteSpace(source[index - 1]))
					{
						if (char.IsLetterOrDigit(source[index + 1]) || source[index + 1] == '_')
						{
